...
class Candidate_Delete:
    def __init__(self, window):
        ...
        # ============ Sign In label =============================================
        # ========================================================================
        self.sign_in_label = Label(self.lgn_frame, text="Delete Candidate", bg="#040405", fg="#FF3399",
        ...
        # ============================username====================================
        # ========================================================================
        self.username_label = Label(self.lgn_frame, text="Candidate_id", bg="#040405", fg="#4f4e4d",
                                    font=("yu gothic ui", 13, "bold"))
        self.username_label.place(x=550, y=300)

        self.username_entry = Entry(self.lgn_frame, highlightthickness=0, relief=FLAT, bg="#040405", fg="#6b6a69",
                                    font=("yu gothic ui ", 12, "bold"), show="*",insertbackground = '#6b6a69',textvariable=self.cid)
        ...
        # ============================password====================================
        # ========================================================================
        self.password_label = Label(self.lgn_frame, text="Party_id", bg="#040405", fg="#4f4e4d",
                                    font=("yu gothic ui", 13, "bold"))
        self.password_label.place(x=550, y=380)

        self.password_entry = Entry(self.lgn_frame, highlightthickness=0, relief=FLAT, bg="#040405", fg="#6b6a69",
                                    font=("yu gothic ui", 12, "bold"), show="*", insertbackground = '#6b6a69',textvariable=self.pid)
    ...
    def validation(self):
        if self.cid.get()=='':
            tkmb.showerror('Invalid','Please Enter Candidate ID')
        elif self.pid.get()=='':
            tkmb.showerror('Invalid','Please Enter Party ID')
        if len(self.cid.get())!=0 and len(self.pid.get())!=0:
            self.database()
    ...
    def database(self):
        try:
            ...
            cur=conn.cursor()
            s='SELECT FName,LName from candidate where Cid=%s and Pid=%s'
            data=(self.cid.get().upper(),self.pid.get().upper())
            cur.execute(s,data)
            res=cur.fetchone()
            if cur.rowcount>0:
                r=tkmb.askquestion("Confirm","Candidate Name:{}\nAre you sure?".format(res[0]+' '+res[1]))
                if r=='yes':
                    q='DELETE from candidate where Cid=%s and Pid=%s'
                    cur.execute(q,data)
                    tkmb.showinfo('DELETE','Candidate Deleted')
                    self.flag=1
            else:
                tkmb.showerror('Error','Invalid Candiate ID or Party ID')
            conn.commit()
            conn.close()
            self.clear()
